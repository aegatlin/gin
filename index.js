          ...
          if (val !== null && option.parseArg) {
            try {
              ...
              if (err.code === "commander.invalidArgument") {
                const message2 = `${invalidValueMessage} ${err.message}`;
        ...
        const myParseArg = (argument, value, previous) => {
          ...
          if (value !== null && argument.parseArg) {
            try {
              ...
              if (err.code === "commander.invalidArgument") {
                const message2 = `error: command-argument value '${value}' is invalid for argument '${argument.name()}'. ${err.message}`;
                this.error(message2, { exitCode: err.exitCode, code: err.code });
              }
              throw err;
            }
...
program.command("prettier").description("install prettier as a dev dependency and create .prettierrc file").action(() => {
  message("installing prettier");
  (0, import_node_child_process.exec)("npm i -D prettier");
  message("overwriting .prettierrc");
  const prettier = (0, import_node_fs.readFileSync)("./reference/.prettierrc");
  (0, import_node_fs.writeFileSync)(".prettierrc", prettier);
});
program.parse();
